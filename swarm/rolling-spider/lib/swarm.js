...
Swarm.prototype.isMember = function (peripheral) {
  ...
  this.logger('targets: ', this.targets);
  if (this.targets.length === 0) {
    ...
    //this.logger(this.targets, localName);
    //this.logger(this.targets, peripheral.uuid);
    // in target list
...
Swarm.prototype.assemble = function (callback) {
  ...
  var onSetup = function () {
    ...
    this.logger('Swarm size is ' + incr + '/' + this.targets.length);
  ...
  this.ble.on('discover', function (peripheral) {
    ...
    if (isSwarmMember) {
      ...
      setTimeout(function () { // Jeløya: Trenger denne timeouten for at Noble skal fortsette å lete etter andre droner.
        this.logger('Timeout');
...
Swarm.prototype.takePicture = broadcast('takePicture');
